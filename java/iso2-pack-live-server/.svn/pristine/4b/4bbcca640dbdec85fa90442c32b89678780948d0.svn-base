...
public class Client 
{
   ...
   static int BUFFSIZE = 4096;         // how many bytes our incoming buffer can hold
   ...
   public String RecvString(int iLength) throws IOException
   {
		char c;
		String out;
		byte [] recByte = new byte[iLength];
		int count = 0;
		...
		out = new String("");
		count = input.read(recByte,0,iLength);
System.out.println("Size : " + count);
		for (int outputCount = 0; outputCount < count; outputCount++)
		{
			c = (char)recByte[outputCount];
